...
#define BLE_UUID_NUS_UP_CHARACTERISTIC 0x0004                      /**< The UUID of the RX Characteristic. */


#define BLE_NUS_MAX_RX_CHAR_LEN        512      /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_NUS_MAX_TX_CHAR_LEN        512      /**< Maximum length of the TX Characteristic (in bytes). */
#define BLE_NUS_MAX_UP_CHAR_LEN        20   //A CHANGER
...
static uint32_t up_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    ble_uuid.type = p_nus->uuid_type;
    ble_uuid.uuid = BLE_UUID_NUS_UP_CHARACTERISTIC;
    ...
    attr_char_value.init_len  = sizeof(uint8_t);
    attr_char_value.init_offs = 0;
    attr_char_value.max_len   = BLE_NUS_MAX_UP_CHAR_LEN;
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    err_code = up_char_add(p_nus, p_nus_init);
    VERIFY_SUCCESS(err_code);
...
uint32_t ble_nus_string_send(ble_nus_t * p_nus, uint8_t * p_string, uint16_t length, bool up)
{
    ...
    if(up)
        hvx_params.handle = p_nus->up_handles.value_handle;
