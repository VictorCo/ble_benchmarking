...
#define BLE_NUS_MAX_RX_CHAR_LEN        BLE_NUS_MAX_DATA_LEN + 39       /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_NUS_MAX_TX_CHAR_LEN        BLE_NUS_MAX_DATA_LEN + 39       /**< Maximum length of the TX Characteristic (in bytes). */
...
{
	SEGGER_RTT_printf(0, "evt type : %d\n", p_ble_evt->header.evt_id);
...
uint32_t ble_nus_string_send(ble_nus_t * p_nus, uint8_t * p_string, uint16_t length)
{
    ...
    VERIFY_PARAM_NOT_NULL(p_nus);
    //SEGGER_RTT_printf(0, "conn : %d\n", p_nus->conn_handle);
    //SEGGER_RTT_printf(0, "notif : %d\n", p_nus->is_notification_enabled);
